...
def home():
    ...
    sample['Temp'] = st.sidebar.slider("Temperature (C)?", -20.0, 40.0, 25.0, 1.0)
    sample['Hum'] = st.sidebar.slider("Humidity (%)?", 0, 100, 40, 1)
    sample['Wind'] = st.sidebar.slider("Wind speed (m/s)?", 0.0, 10.0, 2.5, 0.1)
    sample['Visb'] = st.sidebar.slider("Visibility (10m)?", 10, 2000, 1500, 1)
    sample['Rainfall'] = st.sidebar.slider("Rainfall (mm)?", 0.0, 40.0, 0.0, 0.1)
    sample['Snow'] = st.sidebar.slider("Snowfall (cm)?", 0.0, 10.0, 0.0, 0.1)
